...
class Cypress {
  ...
  public function Theming() {
    ...
    add_action('wp_head', function(){global $wp_the_query; if ($id = $wp_the_query->get_queried_object_id()) : $data = get_post($id); $type = ''; if($data) $type = $data->post_type; if($type && $type == 'page') : echo '<link rel="canonical" href="' . get_permalink( $id ) . '">'; elseif($type && $type == 'post') : $date = new DateTime($data->post_date); $category = get_the_category($id)[0]->slug; $path = 'articles' . $date->format('/d/m/Y/') . $category . '/' . $data->post_name; echo '<link rel="canonical" href="' . home_url($path) . '">'; endif; endif; });
    ...
    add_filter('post_thumbnail_html', function($html, $post, $id, $size, $attr) {if( !$attr ) $attr = array(); if( !array_key_exists('class', $attr) ) $attr = array('class' => 'default'); $class = ''; $class .= ' ' . $attr['class']; $props = ''; foreach ($attr as $key => $value) { if($key !== 'class' ) $props .= ' '.$key.'="'.$value.'"'; }; if( $this->cypress_supports('lazy-load') && preg_match('#lazy#', $attr['class']) ) : $html = '<img data-src="' . wp_get_attachment_image_src($id, $size)[0] . '" title="' . get_the_title($id) . '" alt="' . get_the_title($id) . ' in ' . get_the_title($post) . ' - ' . get_bloginfo('name') . '" width="' . wp_get_attachment_image_src($id, $size)[1] . '" height="' . wp_get_attachment_image_src($id, $size)[2] . '" class="thumbnail ' . $size . $class . '"'.$props.' />'; else : $html = '<img src="' . wp_get_attachment_image_src($id, $size)[0] . '" title="' . get_the_title($id) . '" alt="' . get_the_title($id) . ' in ' . get_the_title($post) . ' - ' . get_bloginfo('name') . '" width="' . wp_get_attachment_image_src($id, $size)[1] . '" height="' . wp_get_attachment_image_src($id, $size)[2] . '" class="thumbnail ' . $size . $class . '"'.$props.' />'; endif; return $html; }, 10, 5);
    ...
    add_filter('body_class', function($class) {global $post; $class = []; if( is_page() && !is_front_page() ) : $class[] = 'page'; elseif( is_single() ) : $class[] = 'single'; elseif( is_front_page() ) : $class[] = 'home'; elseif( is_archive() ) : $class[] = 'archive'; elseif( is_search() ) : $class[] = 'search'; elseif( is_404() ) : $class[] = '404'; else: $class[] = get_post_type(); endif; if( is_page() && $post->post_parent ) : $parents = get_post_ancestors( $post ); $i = 0; foreach ($parents as $parent ) : if($i == 0) $class[] = 'parent-' . get_post($parent)->post_name; $i++; endforeach; endif; if( is_page() ) : $template = get_page_template_slug($post->ID); if($template) $class[] = basename($template, '.php'); endif; return $class; });
  ...
  private function cypress_support($feature, $key = false) {
    $support = get_theme_support($feature)[0];
    if( $key ) :
      if( array_key_exists($key, $support) ) : $support = $support[$key]; else: $support = false; endif;
    endif;
